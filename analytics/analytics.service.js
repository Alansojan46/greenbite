...
import { predictDonationVolume } from "../server/services/aiAnalytics.service.js";
...
export const expiryRisk = async () => {
  // The project does not store an "urgencyScore" field on donations.
  // We use spoilageRisk as the urgency/expiry proxy without altering schema.
...
export const ngoPerformance = async () => {
  // Compute performance based on donation.claims[] without changing schema.
  // - response time: first claim time - donation createdAt (per donation per NGO)
  // - total claims: sum of claim entries for NGO
  // - completion rate: completed donations / unique donations claimed
  const rows = await Donation.aggregate([
